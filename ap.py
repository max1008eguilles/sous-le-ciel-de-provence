...
if check_password():
   ...
   # --- PAGE RÉSERVATIONS ---
    ...
    # --- PAGE DÉTAIL 119 ---
        ...
        resa_119 = df_resa[df_resa["Appartement"].astype(str).isin(["119", 119])].copy()
        last_day_selected_month = (date(sel_year, sel_month % 12 + 1, 1) - timedelta(days=1)) if sel_month < 12 else date(sel_year, 12, 31)
        ...
        for _, r in resa_119.iterrows():
            ...
            while curr < d_dep:
                if curr.year == sel_year:
                    real_an_cumule += (float(r["Montant"]) / (d_dep - d_arr).days)
                    nuits_an_cumule += 1
                if curr in month_data:
                    month_data[curr]["montant"] = float(r["Montant"]) / (d_dep - d_arr).days
                    month_data[curr]["client"] = r["Prénom_Nom"]
                curr += timedelta(days=1)
            if d_dep in month_data:
                month_data[d_dep]["auto_menage"] = True
        ...
        for d, v in month_data.items():
            ...
            rows.append({"Date_Full": d_str, "Date": d.strftime("%d/%m"), "Ménage": is_checked})
        
        df_display = pd.DataFrame(rows)
        edited_df = st.data_editor(
            df_display.style.apply(lambda row: ['background-color: #2E8B57; color: white'] * len(row) if row["Ménage"] else [''] * len(row), axis=1),
        ...
        if st.button("💾 Enregistrer le planning 119"):
            df_new = pd.DataFrame({"Date": edited_df["Date_Full"], "Etat": edited_df["Ménage"]})
            try:
                df_global = pd.read_sql("menages_manuels_119", conn.engine)
                df_global["Date"] = pd.to_datetime(df_global["Date"]).dt.strftime('%Y-%m-%d')
                df_global = df_global[~df_global["Date"].isin(df_new["Date"])]
            ...
            st.success("Données sauvegardées et fusionnées !")
            st.rerun()
