...
if check_password():
    ...
    # --- CHARGEMENT DES DONNÉES (VERSION AVANCÉE) ---
    def load_config():
    ...
    def load_compta():
        if os.path.exists(COMPTA_FILE):
            ...
            # S'assurer que Justificatif existe
            if "Justificatif" not in df.columns: df["Justificatif"] = False
            return df
    ...
    def load_resa():
        if os.path.exists(RESA_FILE):
            ...
            # Rétablir toutes les colonnes de tes captures d'écran
    ...
    if page == "RNM IMMO":
        ...
        if not df_cfg.empty:
            fig = px.bar(df_cfg, x="Bien", y=["Patrimoine Net Bien", "Capital Restant"], 
                         barmode="stack", color_discrete_map={"Patrimoine Net Bien": "#7030A0", "Capital Restant": "#E1E1E1"})
    ...
    # --- PAGE COMPTA ---
        ...
        c2.metric("Cash", f"{solde_cash:,.2f} €")
        c3.metric("TOTAL", f"{total_treso:,.2f} €")
        
        st.divider()
        st.subheader("📝 Journal complet")
        # On affiche tout, y compris le justificatif
    ...
    # --- PAGE RÉSERVATIONS ---
        ...
        # Affichage avec toutes les colonnes (Tél, Mail, Codes)
        ed_res = st.data_editor(df_resa, num_rows="dynamic", use_container_width=True)
        
        if st.button("💾 Sauvegarder Réservations"):
            df_s = ed_res.copy()
            # Nettoyage date 2025/2026 automatique
            def fix_y(row):
        ...
        # Calendrier
        evts = []
        for _, r in ed_res.iterrows():
            try:
                evts.append({"title": f"[{r['Appartement']}] {r['Prénom_Nom']}", "start": str(r["Date Arrivée"]), "end": str(r["Date Départ"]), "allDay": True})
            except: continue
    ...
    # --- PAGE DÉTAIL 014 ---
        ...
        df_obj_y = df_obj_all[df_obj_all["Année"] == sel_year].copy()
        if df_obj_y.empty:
        ...
        for _, r in r014.iterrows():
            try:
                da, dd, mt = r["Date Arrivée"], r["Date Départ"], float(r["Montant"])
                delta = (dd - da).days
                if delta > 0:
                    pxn = mt / delta
                    curr = da
        ...
        m2.metric("Objectif", f"{obj_m:,.0f} €")
        m3.metric("% R/O", f"{(real_m/obj_m*100 if obj_m > 0 else 0):.1f}%")
