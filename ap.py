...
if submit:
    new_data = pd.DataFrame([[str(date), bien, type_trans, montant, desc]], columns=df.columns)
...
# --- DASHBOARD ---
if not df.empty and len(df["Bien"].dropna()) > 0:
    # Filtres
    liste_biens = ["Tous"] + sorted([b for b in df["Bien"].unique() if b is not None])
    ...
    loyers = data_vue[data_vue["Type"] == "Loyer"]["Montant"].sum()
    charges = data_vue[data_vue["Type"] != "Loyer"]["Montant"].sum()
    
    c1.metric("Revenus", f"{loyers:.2f} €")
    c2.metric("Charges", f"{charges:.2f} €", delta_color="inverse")
    c3.metric("Bénéfice", f"{loyers - charges:.2f} €")

    # Graphiques
    st.divider()
    col_left, col_right = st.columns(2)
    with col_left:
        fig_pie = px.pie(data_vue, values='Montant', names='Type', title="Dépenses vs Revenus")
        st.plotly_chart(fig_pie, use_container_width=True)
    with col_right:
        st.subheader("Historique")
        st.dataframe(data_vue, use_container_width=True)
else:
    st.warning("⚠️ Aucune donnée exploitable. Ajoute ta première transaction dans le menu à gauche !")
