...
if check_password():
    ...
    # --- PAGE COMPTA (RESTAURATION COMPLÈTE) ---
    elif page == "COMPTA":
    ...
    # --- PAGE RÉSERVATIONS (COULEURS : 014=BLEU, 119=VERT) ---
        ...
        for _, r in edited_resa.iterrows():
            if pd.notnull(r["Date Arrivée"]) and pd.notnull(r["Date Départ"]):
                color = "#1E90FF" if "014" in str(r["Appartement"]) else "#2E8B57" if "119" in str(r["Appartement"]) else "#808080"
                evts.append({"title": f"[{r['Appartement']}] {r['Prénom_Nom']}", "start": str(r["Date Arrivée"]), "end": str(r["Date Départ"]), "color": color, "allDay": True})
